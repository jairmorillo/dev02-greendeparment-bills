...
<div class="row">
     <div class="col-md-12">
      ...
      <input type="hidden" class="form-control" name="idt" id="idt"  value="{{ $id }}"/>


         <div class="form-group">
         <label for="country_name">Bill Number:</label>
         <input type="text" class="form-control" name="payment_bill_code" id="payment_bill_code" value="" readonly/>
         </div>          
...
<script type="text/javascript">
 $('#amount').mask('000.000.000.000.000,00', {reverse: true});
  ...
  $(function () {
    ...
    const urlState = "{{ route('transaction.index') }}" +'/show/' + idp +'';
                    fetch(urlState, {
                        method: 'GET',
                      })
                      .then(res => res.json())
                      ...
                      console.log(data);
                      $('#payment_bill_id').val(data.payment_bill_id);
                      $('#payment_bill_code').val(data.payment_bill_code);
                      $('#payment_bill_customer').val(data.payment_bill_customer);
                      $('#payment_bill_description').val(data.payment_bill_description);
                      $('#payment_bill_total').val(data.payment_bill_total);
                      $('#payment_partial_payment_mont').val(data.payment_partial_payment_mont);
                      $('#payment_partial_payment_rest').val(data.payment_partial_payment_rest);
                      $('#amount').val(data.amount);
                      $('#payment_transaction_status').val(data.payment_transaction_status);
                      $('#amount').val(data.amount);
